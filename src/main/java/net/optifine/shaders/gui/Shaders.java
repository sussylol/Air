...
import net.optifine.gui.ScreenOF;
...
public class Shaders extends ScreenOF {
    ...
    public Shaders(Screen par1Screen, GameSettings par2GameSettings) {
    ...
    public void initGui() {
        ...
        if (net.optifine.shaders.Shaders.shadersConfig == null) {
            net.optifine.shaders.Shaders.loadConfig();
        }
        ...
        if (button.enabled) {
            if (!(button instanceof GuiButtonEnumShaderOption)) {
                if (!rightClick) {
                    switch (button.id) {
                        case 201:
                            switch (getOSType()) {
                                case 1:
                                    String s = String.format("cmd.exe /C start \"Open file\" \"%s\"", net.optifine.shaders.Shaders.shaderPacksDir.getAbsolutePath());
                                ...
                                case 2:
                                    try {
                                        Runtime.getRuntime().exec(new String[]{"/usr/bin/open", net.optifine.shaders.Shaders.shaderPacksDir.getAbsolutePath()});
                                        return;
                            ...
                            if (flag) {
                                ...
                                Sys.openURL("file://" + net.optifine.shaders.Shaders.shaderPacksDir.getAbsolutePath());
                            }

                            break;

                        case 202:
                            net.optifine.shaders.Shaders.storeConfig();
                            this.saved = true;
                            this.mc.displayGuiScreen(this.parentGui);
                            break;

                        case 203:
                            ShaderOptions guishaderoptions = new ShaderOptions(this, Config.getGameSettings());
                ...
                switch (guibuttonenumshaderoption.getEnumShaderOption()) {
                    case ANTIALIASING:
                        net.optifine.shaders.Shaders.nextAntialiasingLevel(!rightClick);

                        if (this.hasShiftDown()) {
                            net.optifine.shaders.Shaders.configAntialiasingLevel = 0;
                        }
                    ...
                    case NORMAL_MAP:
                        net.optifine.shaders.Shaders.configNormalMap = !net.optifine.shaders.Shaders.configNormalMap;

                        if (this.hasShiftDown()) {
                            net.optifine.shaders.Shaders.configNormalMap = true;
                        }
                    ...
                    case SPECULAR_MAP:
                        net.optifine.shaders.Shaders.configSpecularMap = !net.optifine.shaders.Shaders.configSpecularMap;

                        if (this.hasShiftDown()) {
                            net.optifine.shaders.Shaders.configSpecularMap = true;
                        }
                    ...
                    case RENDER_RES_MUL:
                        net.optifine.shaders.Shaders.configRenderResMul = this.getNextValue(net.optifine.shaders.Shaders.configRenderResMul, QUALITY_MULTIPLIERS, QUALITY_MULTIPLIER_DEFAULT, !rightClick, this.hasShiftDown());
                        net.optifine.shaders.Shaders.uninit();
                        net.optifine.shaders.Shaders.scheduleResize();
                        break;

                    case SHADOW_RES_MUL:
                        net.optifine.shaders.Shaders.configShadowResMul = this.getNextValue(net.optifine.shaders.Shaders.configShadowResMul, QUALITY_MULTIPLIERS, QUALITY_MULTIPLIER_DEFAULT, !rightClick, this.hasShiftDown());
                        net.optifine.shaders.Shaders.uninit();
                        net.optifine.shaders.Shaders.scheduleResizeShadow();
                        break;

                    case HAND_DEPTH_MUL:
                        net.optifine.shaders.Shaders.configHandDepthMul = this.getNextValue(net.optifine.shaders.Shaders.configHandDepthMul, HAND_DEPTH_VALUES, HAND_DEPTH_DEFAULT, !rightClick, this.hasShiftDown());
                        net.optifine.shaders.Shaders.uninit();
                        break;

                    case OLD_HAND_LIGHT:
                        net.optifine.shaders.Shaders.configOldHandLight.nextValue(!rightClick);

                        if (this.hasShiftDown()) {
                            net.optifine.shaders.Shaders.configOldHandLight.resetValue();
                        }
                    ...
                    case OLD_LIGHTING:
                        net.optifine.shaders.Shaders.configOldLighting.nextValue(!rightClick);

                        if (this.hasShiftDown()) {
                            net.optifine.shaders.Shaders.configOldLighting.resetValue();
                        }

                        net.optifine.shaders.Shaders.updateBlockLightLevel();
                        net.optifine.shaders.Shaders.uninit();
                        this.mc.scheduleResourcesRefresh();
                        break;

                    case TWEAK_BLOCK_DAMAGE:
                        net.optifine.shaders.Shaders.configTweakBlockDamage = !net.optifine.shaders.Shaders.configTweakBlockDamage;
                        break;

                    case CLOUD_SHADOW:
                        net.optifine.shaders.Shaders.configCloudShadow = !net.optifine.shaders.Shaders.configCloudShadow;
                        break;

                    case TEX_MIN_FIL_B:
                        net.optifine.shaders.Shaders.configTexMinFilB = (net.optifine.shaders.Shaders.configTexMinFilB + 1) % 3;
                        net.optifine.shaders.Shaders.configTexMinFilN = net.optifine.shaders.Shaders.configTexMinFilS = net.optifine.shaders.Shaders.configTexMinFilB;
                        button.displayString = "Tex Min: " + net.optifine.shaders.Shaders.texMinFilDesc[net.optifine.shaders.Shaders.configTexMinFilB];
                        ShadersTex.updateTextureMinMagFilter();
                        break;

                    case TEX_MAG_FIL_N:
                        net.optifine.shaders.Shaders.configTexMagFilN = (net.optifine.shaders.Shaders.configTexMagFilN + 1) % 2;
                        button.displayString = "Tex_n Mag: " + net.optifine.shaders.Shaders.texMagFilDesc[net.optifine.shaders.Shaders.configTexMagFilN];
                        ShadersTex.updateTextureMinMagFilter();
                        break;

                    case TEX_MAG_FIL_S:
                        net.optifine.shaders.Shaders.configTexMagFilS = (net.optifine.shaders.Shaders.configTexMagFilS + 1) % 2;
                        button.displayString = "Tex_s Mag: " + net.optifine.shaders.Shaders.texMagFilDesc[net.optifine.shaders.Shaders.configTexMagFilS];
                        ShadersTex.updateTextureMinMagFilter();
                        break;

                    case SHADOW_CLIP_FRUSTRUM:
                        net.optifine.shaders.Shaders.configShadowClipFrustrum = !net.optifine.shaders.Shaders.configShadowClipFrustrum;
                        button.displayString = "ShadowClipFrustrum: " + toStringOnOff(net.optifine.shaders.Shaders.configShadowClipFrustrum);
                        ShadersTex.updateTextureMinMagFilter();
                }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        String s = "OpenGL: " + net.optifine.shaders.Shaders.glVersionString + ", " + net.optifine.shaders.Shaders.glVendorString + ", " + net.optifine.shaders.Shaders.glRendererString;
