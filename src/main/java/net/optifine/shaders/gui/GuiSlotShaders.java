...
import net.minecraft.client.gui.YesNo;
...
class GuiSlotShaders extends GuiSlot {
    ...
    final Shaders shadersGui;

    public GuiSlotShaders(Shaders par1GuiShaders, int width, int height, int top, int bottom, int slotHeight) {
    ...
    public void updateList() {
        this.shaderslist = net.optifine.shaders.Shaders.listOfShaders();
        this.selectedIndex = 0;
        int i = 0;

        for (int j = this.shaderslist.size(); i < j; ++i) {
            if (this.shaderslist.get(i).equals(net.optifine.shaders.Shaders.currentShaderName)) {
    ...
    protected void elementClicked(int index, boolean doubleClicked, int mouseX, int mouseY) {
        if (index != this.selectedIndex || this.lastClicked != this.lastClickedCached) {
            ...
            IShaderPack ishaderpack = net.optifine.shaders.Shaders.getShaderPack(s);
    ...
    private void selectIndex(int index) {
        ...
        net.optifine.shaders.Shaders.setShaderPack((String) this.shaderslist.get(index));
            ...
            if (properties == null) {
                ...
                if (s1 == null) {
                    ...
                    if (i >= 0) {
                        ...
                        YesNo guiyesno = new YesNo(guiyesnocallback, s4, s5, 0);
