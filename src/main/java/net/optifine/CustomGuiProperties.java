...
import net.minecraft.client.gui.Enchantment;
import net.minecraft.client.gui.Hopper;
...
public class CustomGuiProperties {
    ...
    private static IWorldNameable getWorldNameable(Screen screen) {
        if (screen instanceof Beacon) return ((Beacon) screen).tileBeacon;
        if (screen instanceof BrewingStand) return ((BrewingStand) screen).tileBrewingStand;
        if (screen instanceof Chest) return ((Chest) screen).lowerChestInventory;
        if (screen instanceof Dispenser) return ((Dispenser) screen).dispenserInventory;
        if (screen instanceof Enchantment) return ((Enchantment) screen).field_175380_I;
        if (screen instanceof Furnace) return ((Furnace) screen).tileFurnace;
        if (screen instanceof Hopper) return ((Hopper) screen).hopperInventory;
        return null;
    }
