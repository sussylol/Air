...
public class EventManager {
    ...
    public static void regOrUnreg(final Object listener, boolean b) {
        if (b) register(listener);
    ...
    public static void register(final Class<? extends IEvent> eventClass, final Object listener) {
        ...
        for (Method method : listenerClass.getDeclaredMethods()) {
            Target target = method.getDeclaredAnnotation(Target.class);
            ...
            for (Class<? extends IEvent> type : target.noParamEvents()) {
                if (eventClass != null && !eventClass.equals(type)) continue;
    ...
    private static void updatePipeline(final Class<? extends IEvent> eventType) {
        ...
        { //Prepare list of all methods and map to map them back to the instance
            ...
            allMethods.sort((o1, o2) -> { //Sort all methods by priority
                Target o1target = o1.getA().getDeclaredAnnotation(Target.class);
                Target o2target = o2.getA().getDeclaredAnnotation(Target.class);
        ...
        { //Insert call method and all listener calls
            ...
            for (int i = 0; i < allMethods.size(); i++) {
                ...
                final Target target = method.getA().getDeclaredAnnotation(Target.class);
                ...
                } else if (ICancellableEvent.class.isAssignableFrom(eventType) && target.skipCancelled()) { //Check if a cancellable event is cancelled and we do not want to listen for it
                ...
                if (ITypedEvent.class.isAssignableFrom(eventType) && !target.type().equals(EnumEventType.ALL)) { //Check if the type of a typed event is the wanted type
                    if (jumpAfter == null) jumpAfter = new Label();
                    ...
                    visitor.visitFieldInsn(Opcodes.GETSTATIC, EnumEventType.class.getName().replace(".", "/"), ReflectUtils.getEnumField(target.type()).getName(), Type.getDescriptor(EnumEventType.class));
