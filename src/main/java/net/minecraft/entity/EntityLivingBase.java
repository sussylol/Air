...
public abstract class EntityLivingBase extends Entity {
     ...
     */
    public float renderYawHead, renderPitchHead;

    /**
     * Entity head rotation at previous tick
     */
    public float prevRenderYawHead, prevRenderPitchHead;
    ...
    public EntityLivingBase(World worldIn) {
        ...
        this.renderYawHead = this.rotationYaw;
    ...
    public void onEntityUpdate() {
        ...
        this.prevRenderYawHead = this.renderYawHead;
     ...
     */
    protected void jump() {
        PlayerJumpEvent event = new PlayerJumpEvent(rotationYaw, this.getJumpUpwardsMotion());
        if (this == MinecraftInstance.mc.player) EventManager.call(event);
        ...
        if (this.isServerWorld()) {
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
                    ...
                    this.strafeToFrom(strafe, forward, f5);
                    f4 = 0.91F;
                    ...
                    double d1 = this.posY;
                    this.strafeToFrom(strafe, forward, 0.02F);
                    this.moveEntity(this.motionX, this.motionY, this.motionZ);
                    this.motionX *= 0.5D;
                    this.motionY *= 0.5D;
                    this.motionZ *= 0.5D;
                ...
                this.strafeToFrom(strafe, forward, f2);
                this.moveEntity(this.motionX, this.motionY, this.motionZ);
                this.motionX *= f1;
    ...
    public void onUpdate() {
        ...
        while (this.renderYawHead - this.prevRenderYawHead < -180.0F) {
            this.prevRenderYawHead -= 360.0F;
        }
    ...
    public void onLivingUpdate() {
        ...
        if (Math.abs(this.motionX) < VersionUtil.getMinimalMotion()) {
            this.motionX = 0.0D;
        }

        if (Math.abs(this.motionY) < VersionUtil.getMinimalMotion()) {
            this.motionY = 0.0D;
        }

        if (Math.abs(this.motionZ) < VersionUtil.getMinimalMotion()) {
            this.motionZ = 0.0D;
        }
    ...
    public Vec3 getLook(float partialTicks) {
        if (partialTicks == 1.0F) {
            return this.getVectorForRotation(this.rotationPitch, this.renderYawHead);
        } else {
            float f = this.prevRotationPitch + (this.rotationPitch - this.prevRotationPitch) * partialTicks;
            float f1 = this.prevRenderYawHead + (this.renderYawHead - this.prevRenderYawHead) * partialTicks;
     ...
     */
    public void setRenderYawHead(float rotation) {
        this.renderYawHead = rotation;
    }
