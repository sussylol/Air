...
import net.optifine.gui.ChatOF;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    private void orientCamera(float partialTicks) {
        ...
        if (!this.mc.gameSettings.debugCamEnable) {
            ...
            if (entity instanceof EntityAnimal) {
                ...
                GlStateManager.rotate(entityanimal.prevRenderYawHead + (entityanimal.renderYawHead - entityanimal.prevRenderYawHead) * partialTicks + 180.0F, 0.0F, 1.0F, 0.0F);
            } else {
                GlStateManager.rotate(entity.prevRotationYaw + (entity.rotationYaw - entity.prevRotationYaw) * partialTicks + 180.0F, 0.0F, 1.0F, 0.0F);
            }
        }
            ...
            for (int l = 0; l < k; ++l) {
                ...
                if (blockpos1.getY() <= blockpos.getY() + i && blockpos1.getY() >= blockpos.getY() - i && biomegenbase.canRain() && biomegenbase.getFloatTemperature(blockpos1) >= 0.15F) {
                    ...
                    if (block.getMaterial() == Material.lava) {
                        this.mc.world.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, (double) blockpos1.getX() + d3, (double) ((float) blockpos1.getY() + 0.1F) - block.getBlockBoundsMinY(), (double) blockpos1.getZ() + d4, 0.0D, 0.0D, 0.0D);
                        ...
                        this.mc.world.spawnParticle(EnumParticleTypes.WATER_DROP, (double) blockpos2.getX() + d3, (double) ((float) blockpos2.getY() + 0.1F) + block.getBlockBoundsMaxY(), (double) blockpos2.getZ() + d4, 0.0D, 0.0D, 0.0D);
                    }
                }
            }
        ...
        if (Config.isSmoothWorld() && Config.isSingleProcessor()) {
            if (this.mc.isIntegratedServerRunning()) {
                ...
                if (integratedserver != null) {
                    ...
                    if (!flag && !(this.mc.currentScreen instanceof DownloadTerrain)) {
                        ...
                        if (this.mc.currentScreen instanceof DownloadTerrain) {
                            Config.sleep(20L);
                        }
    ...
    private void frameInit() {
        ...
        if (this.mc.currentScreen != null && this.mc.currentScreen.getClass() == Chat.class) {
            this.mc.displayGuiScreen(new ChatOF((Chat) this.mc.currentScreen));
        }
    }
