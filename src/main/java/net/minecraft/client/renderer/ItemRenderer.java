    ...
    public void renderItemInFirstPerson(float partialTicks) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            if (this.itemToRender != null) {
                ItemRendererEvent event = new ItemRendererEvent(EnumAction.NONE);
                EventManager.call(event);
                if (this.itemToRender.getItem() instanceof ItemMap) {
                    this.renderItemMap(abstractclientplayer, f2, f, f1);
                } else if (abstractclientplayer.getItemInUseCount() > 0 || event.getAction() != EnumAction.NONE) {
                    EnumAction enumaction = event.getAction() != EnumAction.NONE ? event.getAction() : this.itemToRender.getItemUseAction();
                    ...
                    switch (enumaction) {
                        ...
                        case BLOCK:
                            BlockItemRenderer renderer = new BlockItemRenderer(f, f1, f2, f3);
                            EventManager.call(renderer);
                            if (renderer.isCancelled()) break;
