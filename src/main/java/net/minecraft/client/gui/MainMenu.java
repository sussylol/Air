...
public class MainMenu extends Screen implements GuiYesNoCallback {
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        if (button.id == 5) {
            this.mc.displayGuiScreen(new Language(this, this.mc.gameSettings, this.mc.getLanguageManager()));
        }

        if (button.id == 1) {
            this.mc.displayGuiScreen(new SelectWorld(this));
        }
        ...
        if (button.id == 12) {
            ...
            if (worldinfo != null) {
                YesNo guiyesno = SelectWorld.makeDeleteWorldYesNo(this, worldinfo.getWorldName(), 12);
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        synchronized (this.threadLock) {
            if (this.openGLWarning1.length() > 0 && mouseX >= this.field_92022_t && mouseX <= this.field_92020_v && mouseY >= this.field_92021_u && mouseY <= this.field_92019_w) {
                ConfirmOpenLink guiconfirmopenlink = new ConfirmOpenLink(this, this.openGLWarningLink, 13, true);
