...
public class SelectWorld extends Screen implements GuiYesNoCallback {
    ...
    private SelectWorld.List availableWorlds;
    ...
    public SelectWorld(Screen parentScreenIn) {
    ...
    public void initGui() {
        ...
        try {
            ...
            this.mc.displayGuiScreen(new ErrorScreen("Unable to load worlds", anvilconverterexception.getMessage()));
            return;
        }
        ...
        this.availableWorlds = new SelectWorld.List(this.mc);
        ...
        if (button.enabled) {
            if (button.id == 2) {
                ...
                if (s != null) {
                    ...
                    YesNo guiyesno = makeDeleteWorldYesNo(this, s, this.selectedIndex);
                    this.mc.displayGuiScreen(guiyesno);
                }
            } else if (button.id == 1) {
                this.func_146615_e(this.selectedIndex);
            } else if (button.id == 3) {
                this.mc.displayGuiScreen(new CreateWorld(this));
            } else if (button.id == 6) {
                this.mc.displayGuiScreen(new RenameWorld(this, this.func_146621_a(this.selectedIndex)));
            } else if (button.id == 0) {
                this.mc.displayGuiScreen(this.parentScreen);
            } else if (button.id == 7) {
                CreateWorld guicreateworld = new CreateWorld(this);
     ...
     */
    public static YesNo makeDeleteWorldYesNo(GuiYesNoCallback selectWorld, String name, int id) {
        ...
        return new YesNo(selectWorld, s, s1, s2, s3, id);
    }

    class List extends GuiSlot {
        public List(Minecraft mcIn) {
            super(mcIn, SelectWorld.this.width, SelectWorld.this.height, 32, SelectWorld.this.height - 64, 36);
        }

        protected int getSize() {
            return SelectWorld.this.field_146639_s.size();
        }

        protected void elementClicked(int slotIndex, boolean isDoubleClick, int mouseX, int mouseY) {
            SelectWorld.this.selectedIndex = slotIndex;
            boolean flag = SelectWorld.this.selectedIndex >= 0 && SelectWorld.this.selectedIndex < this.getSize();
            SelectWorld.this.selectButton.enabled = flag;
            SelectWorld.this.deleteButton.enabled = flag;
            SelectWorld.this.renameButton.enabled = flag;
            SelectWorld.this.recreateButton.enabled = flag;

            if (isDoubleClick && flag) {
                SelectWorld.this.func_146615_e(slotIndex);
            }
        }

        protected boolean isSelected(int slotIndex) {
            return slotIndex == SelectWorld.this.selectedIndex;
        }

        protected int getContentHeight() {
            return SelectWorld.this.field_146639_s.size() * 36;
        }

        protected void drawBackground() {
            SelectWorld.this.drawDefaultBackground();
        }

        protected void drawSlot(int entryID, int p_180791_2_, int p_180791_3_, int p_180791_4_, int mouseXIn, int mouseYIn) {
            SaveFormatComparator saveformatcomparator = SelectWorld.this.field_146639_s.get(entryID);
            String s = saveformatcomparator.getDisplayName();

            if (StringUtils.isEmpty(s)) {
                s = SelectWorld.this.field_146637_u + " " + (entryID + 1);
            }

            String s1 = saveformatcomparator.getFileName();
            s1 = s1 + " (" + SelectWorld.this.field_146633_h.format(new Date(saveformatcomparator.getLastTimePlayed()));
            s1 = s1 + ")";
            String s2 = "";

            if (saveformatcomparator.requiresConversion()) {
                s2 = SelectWorld.this.field_146636_v + " " + s2;
            } else {
                s2 = SelectWorld.this.field_146635_w[saveformatcomparator.getEnumGameType().getID()];
            ...
            SelectWorld.this.drawString(SelectWorld.this.fontRendererObj, s, p_180791_2_ + 2, p_180791_3_ + 1, 16777215);
            SelectWorld.this.drawString(SelectWorld.this.fontRendererObj, s1, p_180791_2_ + 2, p_180791_3_ + 12, 8421504);
            SelectWorld.this.drawString(SelectWorld.this.fontRendererObj, s2, p_180791_2_ + 2, p_180791_3_ + 12 + 10, 8421504);
        }
    }
}
