...
public class CreateWorld extends Screen {
    ...
    public CreateWorld(Screen p_i46320_1_) {
        ...
        if (button.enabled) {
            if (button.id == 1) {
                ...
                if (WorldType.worldTypes[this.selectedIndex] == WorldType.FLAT) {
                    this.mc.displayGuiScreen(new CreateFlatWorld(this, this.chunkProviderSettingsJson));
                } else {
                    this.mc.displayGuiScreen(new CustomizeWorldScreen(this, this.chunkProviderSettingsJson));
                }
            }
        }
    }
