...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        this.mc.getTextureManager().bindTexture(ViaLoadingBase.getInstance().getTargetVersion().isNewerThanOrEqualTo(ProtocolVersion.v1_9) ?
                new_icons : icons);
        GlStateManager.enableBlend();

        if (this.showCrosshair() && !ViaLoadingBase.getInstance().getTargetVersion().isNewerThanOrEqualTo(ProtocolVersion.v1_9)) {
    ...
    {
        if (ViaLoadingBase.getInstance().getTargetVersion().isOlderThan(ProtocolVersion.v1_9)) return;
...
//                }
            ...
            else
            {
                GlStateManager.tryBlendFuncSeparate(775, 769, 1, 0);
                GlStateManager.enableAlpha();
                if (this.showCrosshair()) this.drawTexturedModalRect(p_184045_2_.getScaledWidth() / 2 - 7, p_184045_2_.getScaledHeight() / 2 - 7, 0, 0, 16, 16);
                this.drawTexturedModalRect(l / 2 - 7, i1 / 2 - 7, 0, 0, 16, 16);

                float f = this.mc.player.getCooledAttackStrength(0.5F);
                boolean flag = false;
