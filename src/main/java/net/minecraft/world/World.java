...
public abstract class World implements IBlockAccess {
    ...
    private boolean isValid(BlockPos pos) {
        return pos.getX() >= -30000000 &&
                ...
                pos.getY() >= getMinHeight() && pos.getY() < getHeight();
    }
    ...
    private boolean isAreaLoaded(int xStart, int yStart, int zStart, int xEnd, int yEnd, int zEnd, boolean allowEmpty) {
        if (yEnd >= getMinHeight() && yStart < getHeight()) {
            xStart = xStart >> 4;
            zStart = zStart >> 4;
            xEnd = xEnd >> 4;
            zEnd = zEnd >> 4;
...
//        }
    ...
    public BlockPos getTopSolidOrLiquidBlock(BlockPos pos) {
        ...
        for (blockpos = new BlockPos(pos.getX(), chunk.getTopFilledSegment() + 16, pos.getZ()); blockpos.getY() >= getMinHeight(); blockpos = blockpos1) {
    ...
    public boolean canBlockFreeze(BlockPos pos, boolean noWaterAdj) {
        ...
        if (!(f > 0.15F)) {
            if (pos.getY() >= getMinHeight() && pos.getY() < getHeight() && this.getLightFor(EnumSkyBlock.BLOCK, pos) < 10) {
                IBlockState iblockstate = this.getBlockState(pos);
                Block block = iblockstate.getBlock();
    ...
    public boolean isBlockPowered(BlockPos pos) {
        return this.getRedstonePower(pos.down(), Direction.DOWN) > 0 || (this.getRedstonePower(pos.up(), Direction.UP) > 0 || (this.getRedstonePower(pos.north(), Direction.NORTH) > 0 || (this.getRedstonePower(pos.south(), Direction.SOUTH) > 0 || (this.getRedstonePower(pos.west(), Direction.WEST) > 0 || this.getRedstonePower(pos.east(), Direction.EAST) > 0))));
    }
    ...
    public int getActualHeight() {
        return this.provider.getHasNoSky() ? 128 : getHeight();
    }
